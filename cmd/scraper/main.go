...
func (s *Scraper) Run() error {
	...
	for _, source := range s.config.Sources {
		scraper := scrape.NewFeedScraper(source, &s.compressionLock, s.config.BucketName, s.config.BucketPathPrefix, s.config.WorkingDirectory, s.config.KafkaAddress, s.config.KafkaStaticTopic)
